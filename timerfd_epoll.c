/*************************************************************************
	> File Name: timerfd_epoll.c
	> Author:jieni 
	> Mail: 
	> Created Time: 2020年10月19日 星期一 23时00分28秒
 ************************************************************************/
...
void *strt_eplth(void *arg)
{
    ...
    while(1){
        ...
        for(i = 0; i < n;i++){
            if (evnts[i].data.fd & EPOLLIN) printf("*********\n");
...
int create_timerfd(struct itimerspec *its,time_t interval)
{
    ...
    its->it_value.tv_nsec = nw.tv_nsec;
...
int main()
{
    ...
    struct epoll_event ev, evnts[MX_EVNTS];
    ev.events = EPOLLIN | EPOLLET;
    ev.data.fd = tfd;
    ...
    while(1){
        int n = epoll_wait(eplfd,evnts,MX_EVNTS,10000);
        if(n == -1){
            perror("epoll_wait() error");
            break;
        }else if(n == 0){
        ...
        for(int i = 0; i < n;i++){
            printf("***** %d\n", i);
            //if (evnts[i].events & EPOLLIN) printf("*********\n");
            printf("%d %d\n", evnts[i].data.fd , tfd);
            if (evnts[i].data.fd == tfd) printf("!!!!!!!!!\n");
            struct param *pm = (struct param *)(evnts[i].data.ptr);
            printf("tfd: %d\ninitial expiration: %ld\ninterval: %ld\n\n",
                pm->tfd,
                (long)(pm->its.it_value.tv_sec),
